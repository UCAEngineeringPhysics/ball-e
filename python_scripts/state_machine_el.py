...
def handle_fixed_ball(ud) -> None:
    ...
    if dist_remaining <= GOAL_THRESHOLD:
        print(f"\n[INFO] Bucket Zone Reached: ({ud.odom_x:.1f}, {ud.odom_y:.1f})")
        ud.mode = "detect_ball"
        ud.fixed_travel_counter = 0
        ud.latest_msg = build_msg(0.0, 0.0, 0, 0, 0)
        return
...
def handle_fixed_bucket(ud) -> None:
    """
    Drives toward the bucket corner (0, 15) feet.
    Replaces FIXED_BUCKET_TRAVEL_FRAMES with spatial coordinates.
    """
    # 1. Target in FEET
    TARGET_X = 0.0
...
def handle_swivel_small_left(ud) -> None:
    """
    Rotates the robot to face the corner at (0, 15) feet using Odometry.
    Once aligned, transitions to fixed_bucket.
    """
    ...
    # 4. Heading error with shortest-path wrap-around
    heading_error = target_th - ud.odom_th
    while heading_error > pi: heading_error -= 2 * pi
    while heading_error < -pi: heading_error += 2 * pi

    # 5. Check if alignment is reached
    if abs(heading_error) < ANGLE_TOLERANCE:
        print(f"\n[INFO] Swivel Complete. Facing {TARGET_X, TARGET_Y}. Error: {heading_error:.2f}")
        ud.mode = "fixed_bucket"
        ud.fixed_travel_counter = 0
        ud.latest_msg = build_msg(0.0, 0.0, 0, 0, 0)
        return

    # 6. Apply turn command
    # No linear velocity (spinning in place)
    ang_cmd = _clamp(heading_error * ANG_KP, -MAX_ANG, MAX_ANG)
