...
#*******************************         ARM          ****************************************************
                ...
                if state.arm_state == "lower":
                    ...
                    sleep(0.1)
                    if navigator.goal_status == 1:
                        state.mode = "bucket"  # "pause" for testing
...
#*******************************         MOTORS          ****************************************************
                ...
                # ************************************************************************
                if state.counter == 1:
                    state.ball_color = "blue_ball"
                    state.way_point_x = 4.0
                    state.way_point_y = 4.0
                elif state.counter == 3:  #mid point
                    state.ball_color = "blue_ball"
                    state.way_point_x = 6.0
                    state.way_point_y = 3.3
                elif state.counter == 4: 
                    state.ball_color = "red_ball"
                    state.way_point_x = 5.3
                    state.way_point_y = 4.0
                elif state.counter == 6:  
                    state.ball_color = "yellow_ball"
                    state.way_point_x = 5.3
                    state.way_point_y = 5.3
                elif state.counter == 8:
                    state.ball_color = "green_ball"
                    state.way_point_x = 4.0
                    state.way_point_y = 5.3
                elif state.counter == 9:  #mid point
                    state.ball_color = "red_ball"
                    state.way_point_x = 3.7
                    state.way_point_y = 4.3
                ...
                if not state.targeting_active:
                    print("Setting waypoint for ball...")
                    navigator.set_goal(
                        state.way_point_x, state.way_point_y, 1700000, 1500000
                ...
                process_targeting(
                    navigator, depth_frame, detections, state.ball_color, 1700000, 1500000
                )

                # 3. Robot has arrived at ball, switch modes
                if navigator.is_goal_reached and state.counter not in [3,9]:
                ...
                # ************************************************************************
                if state.counter == 2:
                    state.bucket_color = "blue_bucket"
                    state.way_point_x = 1.0
                    state.way_point_y = 1.0
                elif state.counter == 5:  #mid point
                    state.bucket_color = "red_bucket"
                    state.way_point_x = 8.3
                    state.way_point_y = 8.7
                elif state.counter == 7: 
                    state.bucket_color = "yellow_bucket"
                    state.way_point_x = 1.0
                    state.way_point_y = 8.3
                elif state.counter == 10:  
                    state.bucket_color = "green_bucket"
                    state.way_point_x = 8.3
                    state.way_point_y = 1.0
                    
                # 1. Set first way point - targeting_active is to help prevent resetting to OG way point during obj detection
                if not state.targeting_active:
                    print("Setting waypoint for bucket...")
                    navigator.set_goal(
                        state.way_point_x, state.way_point_x, 1000000, 1500000
                    )  # Coordinates for first way point
                    state.targeting_active = True
                    # sleep(0.1)
                ...
                # 2. Use obj detection (for *ball* specifically) to improve/update way point
                process_targeting(
                    navigator, depth_frame, detections, state.bucket_color, 1000000, 1500000
                )

                # 3. Robot has arrived at ball, switch modes
                if navigator.is_goal_reached:
                    print("Arrived at bucket. Switching to drop.")
                    state.mode = "drop"
                    state.arm_state = "idle"
                    state.targeting_active = False
                    state.counter += 1
