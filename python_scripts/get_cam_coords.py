...
def transform_cam_to_odom(point_camera, robot_pose):
    """
    Transforms a 3D point from the camera frame to the odom frame.

    Args:
        point_camera (list or tuple): object coords - (x_c, y_c, z_c) representing the object in the camera frame.
        robot_pose (list or tuple): (X, Y, theta) representing the robot's current odometry.
    Returns:
        numpy.ndarray: [x_o, y_o, z_o] representing the object in the odom frame.
    """
    # 1. Parse inputs into numpy arrays
    p_c = np.array(point_camera)
    X, Y, theta = robot_pose

    # 2. Static Transformation: camera_link to base_link
    R_c_b = np.array([[0, 0, 1], [-1, 0, 0], [0, -1, 0]])
    t_c_b = np.array([-0.08, 0.0, 0.2])  # Your specific camera mount offset
            ...
            for label, conf, bbox in detections:
                ...
                if depth_in_meters > 0:
                    ...
                    point = rs.rs2_deproject_pixel_to_point(
                        intrinsics, [cx, cy], depth_in_meters
                    )

                    print(
                        f"Object: {label} at X:{point[0]:.2f}m, Y:{point[1]:.2f}m, Z:{point[2]:.2f}m"
                    )
                    global_coords = transform_cam_to_odom(
                        (point[0], point[1], point[2]), (0, 0, 0)
                    )
                    print(f"object coords under odom: {global_coords}")
